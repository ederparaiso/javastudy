...
public class ConversionServiceClient {
	static final String REST_URI = "http://localhost:8080/RestServer";
	...
	public static void main(String[] args) {
		...
		System.out.println(ConversionServiceClient.inchToFeet(50));
	...
	public static String inchToFeet(int inch) {
		ClientConfig config = new DefaultClientConfig();
		Client client = Client.create(config);
		WebResource service = client.resource(REST_URI);

		WebResource addService = service.path("rest").path(INCH_TO_FEET + inch);
		return getOutputAsXML(addService);
	}
}